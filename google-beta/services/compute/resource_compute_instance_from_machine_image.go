...
func computeInstanceFromMachineImageSchema() map[string]*schema.Schema {
	...
	s["source_machine_image_encryption_key"] = &schema.Schema{
		...
		Optional: true,
		MaxItems: 1,
		Description: `Optional encryption key fields for the source machine image." + 
			"Supported fields: raw_key, rsa_encrypted_key, kms_key_name, sha256, kms_key_service_account."`,
...
func resourceComputeInstanceFromMachineImageCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("source_machine_image_encryption_key"); ok {
		list := v.([]interface{})
		if len(list) > 0 && list[0] != nil {
			instance.SourceMachineImageEncryptionKey = expandInstanceSourceMachineImageEncryptionKey(list[0].(map[string]interface{}))
		} else {
			return fmt.Errorf("source_machine_image_encryption_key must be provided")
		}
	} else {
		return fmt.Errorf("source_machine_image_encryption_key is missing")
	}
...
func expandInstanceSourceMachineImageEncryptionKey(m map[string]interface{}) *compute.CustomerEncryptionKey {
	key := &compute.CustomerEncryptionKey{}

	if raw, ok := m["raw_key"].(string); ok && raw != "" {
		key.RawKey = raw
	}
	if rsa, ok := m["rsa_encrypted_key"].(string); ok && rsa != "" {
		key.RsaEncryptedKey = rsa
	}
	if kms, ok := m["kms_key_name"].(string); ok && kms != "" {
		key.KmsKeyName = kms
	}
	if sa, ok := m["kms_key_service_account"].(string); ok && sa != "" {
		key.KmsKeyServiceAccount = sa
	}
	return key
}
